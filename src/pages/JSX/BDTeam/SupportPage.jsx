...
export default function OnlineSupport() {
  ...
  const handleSend = async () => {
    if (!message.trim() || !selectedTicket || selectedTicket.status === "CLOSED")
      return;

    try {
      const res = await fetch(
        `${config.baseURL}/support/threads/${selectedTicket.id}/messages/`,
        {
          method: "POST",
  ...
  const handleClose = async () => {
    ...
    try {
      const res = await fetch(
        `${config.baseURL}/support/threads/${selectedTicket.id}/close/`,
        { method: "POST" }
      );
